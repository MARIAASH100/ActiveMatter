...
def calculate_gr(file_path, Dr, max_r, arena_area, num_particles_real, fps):
    # Load the CSV file
    ...
    # Average density constant across frames=> assumption
    rho_0 = num_particles_real / arena_area  # Average density
    ...
    for frame_idx in range(len(data)):
        ...
        if num_particles < 2:
            continue  # Skip frames with less than 2 particles=> if they flip over it is a problem
        ...
        #print(len(distances)) => debuggig: suppose N particles in a frame suppose to be (N-1)*N
        #print(len(distances)) => debugging
        ...
        #print(pdf_dij) => debugging
        #print(sum(pdf_dij)) => debugging: indeed the "integral" of PDF gives 1

        # Step 5: Normalize by shell area
        shell_areas = 2 * np.pi * r_bins[:-1] * Dr  # A(r) = 2*pi*r*Dr
        #print(shell_areas) => debugging
        #print(r_bins[:-1]) => debugging
    ...
    # Step 8: Plot g(r) as a function of r => regular plot
    #r_values = r_bins[:-1] + Dr / 2  # Bin centers
    #plt.figure(figsize=(8, 6))
    #plt.plot(r_values, gr_avg, label="g(r)", marker='o')
    ...
    #Histogram:
    # Plot g(r) as a line histogram
    r_values = r_bins[:-1] + Dr / 2  # Bin centers
    r_values=r_values*(1/29)
    plt.figure(figsize=(8, 6))

    # Create a step-like plot to mimic a histogram outline
    plt.step(r_values, gr_avg, where='mid', color='blue', linewidth=1.5)

    # Add labels, title, and legend
    plt.xlabel("r [cm]")
    plt.ylabel("g(r) [unitless]")
    plt.title("N = 6 particles") #Change the number of particles
    plt.legend()
    ...
    return r_values, gr_avg

# Example usage
#file_path = r"C:\\Users\\anna1\\PyCharmProjects\\LAB C - projects\\Active Matter 2\\6p.csv"
file_path = r"C:\\Users\\anna1\PycharmProjects\\LAB C - projects\Active Matter 2\\ideal_gas_fixed_circle_particles.csv"
Dr = 30  # Bin width
max_r = 900  # Maximum distance
arena_area = np.pi*435**2  # Arena area
num_particles_real = 14# Total particles in the system
fps = 1 / 30  # Frame sampling period in seconds

r_values, gr_avg = calculate_gr(file_path, Dr, max_r, arena_area, num_particles_real, fps)